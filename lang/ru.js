export default {
    //SEO
    tithome: 'Amediatv.uz - Первый узбекский фандаб, переведенное аниме',
    titmeta: 'аниме, перевод аниме, забавное аниме, узанимэ, анибла',

    janr: 'Жанры',
    years: 'Годы',
    kirish: 'Вход ',
    tolshart: 'Обязательно',
    davometish: 'Продолжить',
    register: ' Регистрация',
    ism: 'Введите имя',
    email: 'Введите email',
    emailregister: 'Зарегистрируйтесь по электронной почте',
    emaillogin: 'Войти по электронной почте',
    parol: 'Введите пароль',
    parol2: 'Подтвердите пароль',
    parolerror: 'Ошибка подтверждения пароля',
    ism: 'Введите  имя',
    kod: 'Введите код',
    kodsms: 'Код был отправлен на ваш номер телефона по SMS',
    togtol: 'Залить правильно',
    notfound: 'Информации не найдено !!!',
    profil: 'Профиль',
    exit: 'Выход',
    search: 'Поиск...',
    sum: 'сум',

    /// footer
    adres: 'Адрес',
    contact: 'Контакты',
    category: 'Категории',
    sotset: 'Социальные сети',
    allzashita: 'Все права защищены',

    // slider
    see: 'Смотреть',
    all: 'Все',
    more: 'Показать еще',
    news: 'Новости',
    free: 'Бесплатно',
    pul: 'Платный',

    ///filter
    anime: 'Аниме фильмы',
    othernews: 'Другие новости',

    /// getbyid
    info: 'Данные',
    country: 'Страна',
    rejissor: 'Режиссер',
    studiya: 'Студия',
    onejanr: 'Жанр',
    onecat: 'Категория',
    year: 'Год',
    down: 'Скачать',
    reply: 'Ответить',
    send: 'Отправить',
    izoh: 'Комментарий ...',

    /// profile
    changename: 'Переименовать',
    edit: ' Изменить',
    balanstoldir: 'Пополнить баланс',
    summaenter: 'Введите сумму',
    Balans: 'Баланс',
    tarifend: 'Дата окончания тарифа',
    tarifla: 'Тарифы',
    like: 'Избранное',
    obuna: ' Подписка',
    tolash: 'Оплатить',
<<<<<<< HEAD
}
=======
    mablagkam: 'На балансе недостаточно денег для подписки',
    obunasuccess: 'Подписка успешна',
    obunayoqilgan: 'У вас включена подписка',
    obunatasdiqlang: 'Подтвердить подписку',
    bekor: 'Отмена',
    tasdiq: 'Подтвердить',
    minikki: 'минимум 2000 сумов',
    tolovtype: 'Тип платежа',
    oylik: 'месяц',
}
>>>>>>> 094438c30aad419be9d20e188cd087e667cf0b83
